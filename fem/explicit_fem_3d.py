...
vertices = ti.field(dtype=ti.i32, shape=(n_elements, 4))
...
def integrate():
    for p in ti.grouped(x):
        ...
        if x[p][1] < 0.2:
            x[p][1] = 0.2
            v[p][1] = 0
        v[p] += (((-x.grad[p] / node_mass) + ti.Vector([0, -10, 0])) * dt) * math.exp(
                dt * -6)
        x[p] += dt * v[p]
...
for i in range(n_nodes_x):
    for j in range(n_nodes_y):
        t = mesh(i, j)
        x[t] = [0.1 + i * dx * 0.5, 0.7 + j * dx * 0.5 + i * dx * 0.1, 0]
        v[t] = [0, -1, 0]
...
while True:
    ...
    if gui.is_pressed(ti.GUI.LMB):
        sphere[None] = ti.Vector([gui.get_cursor_pos()[0],gui.get_cursor_pos()[1],0])

    gui.circle((sphere[None][0], sphere[None][1], sphere[None][2]), radius=63, color=0x068587)
    ...
    for i in range(n_elements):
        for j in range(3):
            ...
            gui.line((node_x[a][0], node_x[a][1]),
                     ...
                     radius=1,
                     color=0x4FB99F)
    gui.circle(node_x, radius=1.5, color=0x3241f4)
